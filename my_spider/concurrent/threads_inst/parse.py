...
class ParseThread(BaseThread):
    ...
    def working(self):
        ...
        """
        # ----1----
        content, url_type, task_serial = self._pool.get_a_task(TPEnum.HTM_PARSE)
        print(task_serial)
        # ----2----
        ...
        if parse_state > 0:
            ...
            if item is not None:
                self._pool.add_a_task(TPEnum.ITEM_SAVE, (parse_state, url_list, item))
        else:
            self._pool.update_number_dict(TPEnum.HTM_PARSE_FAIL, +1)
            logging.error("%s error: %s, %s", url_list[0], url_list[1],
