...
def segment_search_in_pi(target: str, segment_len: int = 5) -> dict:
    ...
    def segment_search(split_str, segment_len):
        ...
        segments = [
            split_str[i : i + segment_len] for i in range(0, split_str_len, segment_len)
        ]

        for segment in segments:
            start_pos = pi_str.find(
                segment
            )  # if segment not in position_results else position_results[segment]
            if start_pos != -1:
